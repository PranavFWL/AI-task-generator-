...
exports.AIBackendAgent = void 0;
const BaseAgent_1 = require("./BaseAgent");
const GeminiService_1 = require("../services/GeminiService");
class AIBackendAgent extends BaseAgent_1.BaseAgent {
    ...
    addErrorHandling(content, filePath) {
        if (filePath.includes('controller')) {
            // Wrap controller methods in try-catch if not already present
            if (!content.includes('try {') && content.includes('async ')) {
                content = content.replace(/(async \w+\([^)]*\): Promise<void> \{)/g, `$1
    try {`);
                content = content.replace(/(\s+}(?:\s*;)?\s*)$/, `    } catch (error) {
    ...
    addInputValidation(content, filePath) {
        if (filePath.includes('controller') && content.includes('req.body')) {
            ...
            content = content.replace(/(const \{ .* \} = req\.body;)/g, `$1
    ...
    addSecurityFeatures(content, filePath) {
        if (filePath.includes('auth') || filePath.includes('user')) {
            // Add rate limiting for auth endpoints
            if (!content.includes('rateLimit')) {
                content = `import rateLimit from 'express-rate-limit';\n${content}`;
                // Add rate limiting middleware
                content = content.replace(/(router\.\w+\(['"]\w+['"],)/g, `$1 rateLimit({ windowMs: 15 * 60 * 1000, max: 5 }),`);
            }
            // Add password strength validation
            if (content.includes('password') && !content.includes('validatePasswordStrength')) {
                content = content.replace(/(if \(!password\) \{)/g, `if (!password || !validatePasswordStrength(password)) {
    ...
    addLoggingAndMonitoring(content, filePath) {
        if (filePath.includes('controller')) {
            // Add request logging
            content = content.replace(/(async \w+\(req: (?:AuthRequest|Request), res: Response\): Promise<void> \{)/g, `$1
    ...
    addAPIDocumentation(content, filePath) {
        if (filePath.includes('controller')) {
            // Add JSDoc comments for API endpoints
            content = content.replace(/(async \w+\(req: (?:AuthRequest|Request), res: Response\): Promise<void> \{)/g, `/**
  ...
  $1`);
    ...
    enhanceTypeScript(content, filePath) {
        ...
        content = content.replace(/async (\w+)\(/g, 'async $1(');
        return content;
    }
    ...
    generateModelInterface(modelName, content) {
...
export interface Update${modelName}Data {
  // Add update-specific fields
    ...
    generateMiddlewareFile(task, existingFiles) {
...
COMMIT;`,
    ...
    generateTestFile(task, existingFiles) {
...
export const generateToken = (userId: string, additionalPayload?: any): string => {
  ...
  return jwt.sign(payload, config.jwtSecret, {
    ...
    async generateFallbackBackend(task) {
        const files = [];
...
exports.AIBackendAgent = AIBackendAgent;
//# sourceMappingURL=AIBackendAgent.js.map