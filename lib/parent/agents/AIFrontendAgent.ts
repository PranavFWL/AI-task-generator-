...
export class AIFrontendAgent extends BaseAgent {
  ...
  private generatePropsInterface(content: string, componentName: string): string {
  ...
  private inferPropType(content: string, propName: string): string {
    if (content.includes(`props.${propName}(`)) return 'Function';
    if (content.includes(`props.${propName}.map`)) return 'any[]';
    if (content.includes(`props.${propName} &&`)) return 'boolean';
    if (content.includes(`{props.${propName}}`)) return 'string | number';
    return 'any';
  }
  ...
  private enhanceAccessibility(content: string): string {
    let enhanced = content;

    // Add labels for inputs
    enhanced = enhanced.replace(
      /<input([^>]*?)type="([^"]*)"([^>]*?)>/g,
      '<input$1type="$2"$3 aria-label="$2 input">'
    );
  ...
  private addResponsiveDesign(content: string, filePath: string): string {
...
@media (max-width: 480px) {
  ...
  private determineFileType(path: string, content: string): 'component' | 'other' {
  ...
  private generateTypesFile(task: TechnicalTask, files: GeneratedFile[]): GeneratedFile {
...
`;
  ...
  private generateIndexFile(componentFiles: GeneratedFile[]): GeneratedFile {
  ...
  private generateStoryFile(componentFile: GeneratedFile, task: TechnicalTask): GeneratedFile {
...
`;
  ...
  private async generateFallbackComponents(task: TechnicalTask): Promise<GeneratedFile[]> {
  ...
  private getFallbackLoginComponent(): string {
...
export const LoginForm: React.FC<LoginFormProps> = ({ onLogin, isLoading = false }) => {
  ...
  private getFallbackTaskComponent(): string {
